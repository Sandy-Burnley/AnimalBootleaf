var config = {
	...
	"instructions": {
		...
		"contents": "<p><strong>Start Here:</strong> Click on a box to reveal markers for each text or identity attributes listed in the right sidebar. You can select the option ALL ON to reveal all the markers on the map. You can also individually isolate check boxes in the right side panel to reveal markers in a more organized and legible fashion. Markers can overlap one another in any order for users to explore patterns and relationships between identifiers. <br> <strong>Labels:</strong> Identifiers that read as labels are meant to clarify the attribute or text highlighted. For example, if a label is titled with the work of the novel, then the label will reveal specific character names. However, if the label is an attribute of species or sex, the label will desginate from what text that marker originates. <br> <strong>GeoJSON Layers:</strong> Click on these layers to show all unspecified vectors for coinciding text. For example, clicking on Vanity Fair will mark every characters movement throughout the entire novel of Vanity Fair, regardless of specified attributes. <br> <strong>Attributes:</strong> Click on these attributes to hone in on and isolate species and sex markers.</p>"
